...
public class ProductControllerTest {
    ...
    @BeforeEach
    void setup() {
        product2 = new Product(new ProductDto("Headphone", "Great", 60000, "", 4.5, "Multilaser", "P300", "BLACK", Set.of(CategoryEnum.technology, CategoryEnum.audio, CategoryEnum.electronics)));
        product1 = new Product(new ProductDto("Veja", "Limpador", 3000, "", 3.7, "Veja", "Veja", "Blue", Set.of(CategoryEnum.house)));
        product1.setId("1");
        product2.setId("2");
    }
    ...
    public void shouldFindAllProducts() throws Exception {
        ...
        response.andExpect(MockMvcResultMatchers.jsonPath("$[1].id").value(product1.getId()));
        response.andExpect(MockMvcResultMatchers.jsonPath("$[0].name").value(product2.getName()));
        response.andExpect(MockMvcResultMatchers.jsonPath("$[1].name").value(product1.getName()));
